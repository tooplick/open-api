...
1. **Console API `/api/**`** — JWT auth via **Spring Security** (stateless filter chain in `config/SecurityConfig`). `JwtAuthenticationFilter` parses the `Authorization: Bearer <jwt>` header, loads the user through `CustomUserDetailsService`, and stores an `AuthUser` principal in the `SecurityContext`. `/api/auth/login|register|email-code|email-register`, `/api/auth/github/**`, and `/api/settings/public` are `permitAll`; the rest of `/api/**` requires authentication. Responses are wrapped in `common.result.Result<T>`. Business exceptions are handled by `GlobalExceptionHandler` (`@RestControllerAdvice`); auth failures (401) and role denials (403) are written as `Result`-shaped JSON by `SecurityConfig`'s `AuthenticationEntryPoint` / `AccessDeniedHandler`.
...
- **Registration is settings-gated (two methods)**: every flow first requires `register.enabled` (master toggle). **Password** — `POST /api/auth/register` (`UserServiceImpl.register`) also needs `register.password.enabled`. **Email-code** — `register.email.enabled` enables a two-call flow: `POST /api/auth/email-code` (send code) then `POST /api/auth/email-register` (consume code + create user). Both still collect **username + password**; the code only proves email ownership — there is **no password-less account** (login goes through `UsernamePasswordAuthenticationToken` and `user.password` is `NOT NULL`). Codes live in **process memory** (`modules/auth/email/VerificationCodeStore`: 6-digit, 10-min TTL, single-use on success, 30 s send cooldown — single-instance only, no Redis). SMTP config is stored in `system_setting` (admin-editable on the settings page; the password is masked `******` on read and kept on a blank/masked write, mirroring `channel.apiKey`), and a `JavaMailSenderImpl` is **rebuilt per send** by `EmailService`, so config edits take effect without a restart. `register.github.enabled` enables GitHub OAuth login/signup: the backend owns code exchange, keeps short-lived in-memory state/tickets, links users by `user.github_id`, and returns the normal JWT `LoginResponse` via `/api/auth/github/exchange`; GitHub Client Secret is stored in `system_setting` and masked on read. The console `LoginView` has a Login/Register tab; the Register tab nests **Password / Email-code sub-tabs** shown per enabled toggle (it reads `/api/settings/public`) and shows a GitHub login button when the public GitHub toggle is enabled. Email signup adds `email`-related `ResultCode`s (1007–1011) and a non-unique `idx_email` on `user` (logic-delete means no unique constraint — dup-check is imperative); GitHub signup adds nullable `github_id`/`github_login`/`avatar_url` columns with a unique `uk_github_id`.
...
### Module layout
`com.aiopen.platform.modules.<feature>` with `{entity, mapper, dto, service, service.impl, controller}` — the features are `user`, `apikey`, `channel`, `ability`, `relay`, `log`, `setting`, `auth`. The `ability` module is the derived routing layer maintained by `channel` writes; `relay/adaptor` holds the provider adapters and `relay/anthropic` the Anthropic inbound; `setting` is the key-value system-settings store; `auth/email` holds the email-verification-code infrastructure (`VerificationCodeStore` in-memory codes + `EmailService`'s per-send `JavaMailSenderImpl`) consumed by `UserServiceImpl`'s email signup. Cross-cutting code lives in `common/` (Result, exceptions, BaseEntity), `config/` (MyBatis-Plus, JWT props, Spring Security `SecurityConfig`, crypto, and the two startup `CommandLineRunner`s — `DataInitializer` for the admin row and `SettingInitializer` for default settings; the SPA resource handler is in `WebConfig`), and `security/` (`JwtUtil`, `JwtAuthenticationFilter`, `CustomUserDetailsService`, `AuthUser`, `Roles`).
